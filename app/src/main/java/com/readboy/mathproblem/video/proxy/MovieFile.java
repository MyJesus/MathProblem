...
import com.readboy.mathproblem.video.movie.MovieConfig;
import com.readboy.mathproblem.video.db.DatabaseProxy;
import com.readboy.mathproblem.video.db.DatabaseProxy.DataItem;
import com.readboy.mathproblem.video.db.VideoDatabaseInfo;
import com.readboy.mathproblem.video.db.VideoInfoDatabaseProxy;
import com.readboy.mathproblem.video.db.VideoInfoDatabaseProxy.VideoInfoItem;
import com.readboy.mathproblem.video.tools.MediaScannerMonster;

public class MovieFile {
    private static final String TAG = "MovieFile";
...
//        scanFile(context, scalpaths);
    ...
    //该方法可能会导致内存泄露，没有断开连接。
//    public static void scanFile(Context context, String[] scalpaths) {
...
//    }
    ...
    static public boolean saveMoviePlayInfo(String path, long currentPos, long duration) {
        boolean back = false;
        if (path != null) {
            DataItem item = DatabaseProxy.queryItem(path);
            if (item != null) {
    ...
    static public boolean saveVideoInfo(String datapath, String dataname, String dependency, int type, long currentPos, long duration,
        ...
        if (datapath != null && dataname != null && dependency != null) {
            VideoInfoItem item = VideoInfoDatabaseProxy.queryItem(datapath, dataname, dependency, type);
            if (item != null) {
                long current = currentPos;
                int play = 0;
...
//		Log.e("MovieFile", "media encryption: "+encryption + ", mPath: " + path);
    ...
    static public int getMediaType(String path) throws IllegalArgumentException {
        ...
        if (!new File(path).exists()){
            Log.e(TAG, "getMediaType: file not find, path = " + path);
            return type;
        }
        try {
