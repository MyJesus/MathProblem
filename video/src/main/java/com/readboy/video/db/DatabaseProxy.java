...
public class DatabaseProxy {
	...
	private String MD_PATH = VideoDatabaseInfo.DBPATH;
	...
	private int insert(String pathUrl, long position, long duration, int play, int isurl) {
		...
		long rowId = mDb.insert(mTableNames, mTableNames, contentValues);
		if (rowId == (-1)) { // 插入失败,创建表再重试
			createTable();
			rowId = mDb.insert(mTableNames, mTableNames, contentValues);
			if(rowId == (-1)) {
				return (-1);
			}
		}
	...
	private int delete(String pathUrl) {
		...
		if(1 != mDb.delete(mTableNames, "pathUrl = \"" + pathUrl + "\"", null)) {
			return (-1);
		}
		return 0;
	}
	...
	private int deletes(String pathUrl) {
		...
		if(0 == mDb.delete(mTableNames, "pathUrl in (" + pathUrl + ")", null)) {
			return (-1);
		}
		return 0;
	}
	...
	private int update(String pathUrl, long position, long duration, int play, int isurl) {
		...
		long curtime = System.currentTimeMillis();
		contentValues.put("time", curtime);
		int count = mDb.update(mTableNames, contentValues, "pathUrl = \"" + pathUrl + "\"", null);
		if (count == 0 || count ==-1) {
	...
	private Cursor query() {
		...
		return mDb.rawQuery("SELECT * FROM " + mTableNames, null);
	}
	...
	private DataItem query(String pathUrl) {
		...
		Cursor mCursor = mDb.rawQuery("SELECT * FROM " + mTableNames + " WHERE pathUrl = ?", new String[]{pathUrl});
		if (mCursor != null) {
	...
	private ArrayList<DataItem> queryInfos(String pathUrl) {
		...
		Cursor mCursor = mDb.rawQuery("SELECT * FROM " + mTableNames + " pathUrl WHERE pathUrl = ?  ORDER BY time DESC", new String[]{pathUrl});
		if (mCursor != null) {
			int count = mCursor.getCount();
			if (count > 0) {
	...
	private void clear() {
		if (mDb != null) {
			mDb.execSQL("DROP TABLE IF EXISTS " + mTableNames);
		}
	}
	...
	private void createTable() {
		if (mDb != null) {
			String sql = "CREATE TABLE IF NOT EXISTS " + mTableNames + "(pathUrl char("+DB_DATA_MAX_SIZE+"), position long("+DB_MD5_SIZE+"), " + "duration long("+DB_MD5_SIZE+"), " 
